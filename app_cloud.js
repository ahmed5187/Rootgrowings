...
// persistence, full rooms/plants CRUD, uploads with 2MB cap, profile modal,
// centered confirmation modal, and bottom-nav click safe.
//
// Load with: <script type="module" src="./app_cloud.js?v=9"></script>
...
(function init(){
  (async () => {
    ...
    function activate(page){
      ...
      // scroll + make sure no dialog blocks clicks
    ...
    profileForm.addEventListener('submit', async (e) => {
      ...
      try {
        await updateProfile(u, { displayName: newName || null, photoURL: newPhoto });
    ...
    function renderHome(){
      ...
      for(const room of Object.keys(groups)){
        ...
        for(const p of groups[room]){
          ...
          water.onclick=async()=>{ const ref=doc(db,'users',auth.currentUser.uid,'plants',p.id); const now=new Date().toISOString(); await updateDoc(ref,{lastWateredUtc:now,updatedAt:now}); showToast('Watered'); };
          const snooze=document.createElement('button'); snooze.className='btn ghost'; snooze.textContent='Snooze';
          snooze.onclick=async()=>{ const d=new Date(p.nextDueUtc); d.setUTCDate(d.getUTCDate()+1); await updateDoc(doc(db,'users',auth.currentUser.uid,'plants',p.id),{nextDueUtc:d.toISOString(),updatedAt:new Date().toISOString()}); showToast('Snoozed 1 day'); };
    ...
    deletePlantBtn.onclick = async ()=>{ const id=form.dataset.editing; if(!id) return; await deleteDoc(doc(db,'users',auth.currentUser.uid,'plants',id)); dialog.close(); showToast('Plant deleted'); };
