...
function setNextDue(p){
  ...
  n.setUTCDate(n.getUTCDate() + calcIntervalDays(p));
...
function groupByRoom(list){
  const map = {};
  for(const r of rooms) map[r.name] = [];
  for(const p of list){
    const rn = p.location || 'Unassigned';
    if(!map[rn]) map[rn] = [];
    map[rn].push(p);
  }
  return map;
...
editRoomForm.addEventListener('submit', async e => {
  ...
  if(!oldName) return;
  const roomDoc = rooms.find(r => r.name === oldName);
  if(!roomDoc) return;
  const patch = {};
...
removeRoomForm.addEventListener('submit', async e => {
  ...
  const roomDoc = rooms.find(r => r.name === name);
  if(!roomDoc) return;
...
removePlantForm.addEventListener('submit', async e => {
  e.preventDefault();
  const id = new FormData(removePlantForm).get('plantToRemove');
  if(!id) return;
  await deleteDoc(doc(db, 'users', user.uid, 'plants', id));
